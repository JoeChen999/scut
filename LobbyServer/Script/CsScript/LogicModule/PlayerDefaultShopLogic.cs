...
{
    public class PlayerDefaultShopLogic
    {
        private int m_UserId;
        private PlayerDefaultShop m_Shop;
        public PlayerDefaultShopLogic()
        {
        ...
        {
            m_Shop = CacheSet.PlayerDefaultShopCache.FindKey(userId.ToString(), userId);
            m_UserId = userId;
            if (m_Shop == null)
            {
                m_Shop = new PlayerDefaultShop();
                m_Shop.UserId = userId;
                m_Shop.NextRefreshTime = GetNextRefreshTime();
                CacheSet.PlayerDefaultShopCache.Add(m_Shop);
                ResetShopItems();
            }
        }

        public PlayerDefaultShop GetShopInfo()
        {
        ...
        private List<ShopItem> GetNewShopItem(int type)
        {
            ...
            var shopData = CacheSet.RandomShopTable.GetAllData(t => t.CurrencyType == type );
        ...
        private static long GetNextRefreshTime()
        {
            ...
            DateTime nextRefreshTime = new DateTime(dt.Year, dt.Month, dt.Day, dt.Hour, 0, 0);
